...
public class DetailsOrderDataBaseHelper extends SQLiteOpenHelper {
    Context context;
    private static final String TABLE_NAME =  "tbl_detail_barang";
    ...
    public DetailsOrderDataBaseHelper(Context context) {
    ...
    public long insertDetailsOrder(Integer id,Integer harga,String nama, Integer stok,String img){
        SQLiteDatabase db= this.getWritableDatabase();
        ContentValues cv=new ContentValues();
        cv.put(FIELD_ID ,id);
        cv.put(FIELD_HARGA ,harga);
        cv.put(FIELD_NAMA ,nama);
        cv.put(FIELD_STOK ,stok);
        cv.put(FIELD_JUMLAH,1);
        cv.put(FIELD_IMG,img);
